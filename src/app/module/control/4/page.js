...
const SCENES = [
  {
    ...
    pose: 'confident',
    narration:
      'Welcome to Learning Module 4. In this neutral knowledge summary task, please briefly explain one basic brain function concept or the 10% brain myth to another reader based on what you learned in previous modules.',
  },
  {
    id: 'feedback',
    pose: 'celebrating',
    narration:
      'Thank you for your response! Summarizing learning content helps review and consolidate information effectively.',
  },
];
...
export default function ControlModule4Page() {
  ...
  const isValidInput = summaryText.trim().length >= 25;
  ...
  const goNext = async () => {
    ...
    if (isLastScene) {
      ...
      try {
        ...
        console.error('Control Module 4 submission error:', err);
      }
      setFlowState('intervention');
      router.push('/module/control/5');
      return;
    }
    ...
    <div className="flex flex-col gap-6 animate-fadeIn w-full max-w-3xl mx-auto">
      <div className="flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="flex-1">
          ...
          <p style={{ ...STYLES.sub, color: '#006764', fontWeight: 600 }}>
            Neutral Knowledge Summary Task
          </p>
          <SpeechBubble
      ...
      >
        ...
        <h3 style={{ fontSize: '1.15rem', fontWeight: 700, color: '#04284b', marginBottom: '12px', fontFamily: 'var(--font-serif)' }}>
          Explain a Brain Function Concept or Myth to Another Reader
        </h3>

        <p style={{ fontSize: '1rem', color: '#334155', lineHeight: 1.6, marginBottom: '14px' }}>
          Based on the previous modules, please briefly explain one basic brain function knowledge point you learned (such as functional localization or multi-network collaboration) or briefly debunk the 10% brain myth to another reader.
        </p>
        ...
        <div style={{ padding: '14px 18px', backgroundColor: '#f0f7f6', border: '1px solid #99d6d3', borderRadius: '12px', marginBottom: '18px' }}>
          ...
          <ul style={{ fontSize: '0.88rem', color: '#334155', lineHeight: 1.5, margin: 0, paddingLeft: '18px', listStyleType: 'disc' }}>
            <li>Write 2 to 3 concise sentences explaining a basic brain concept or the 10% myth.</li>
            <li>Focus purely on general, neutral neuroscientific information.</li>
            <li>Do not evaluate personal abilities, clinical performance, work stress, or coping strategies.</li>
            <li><em>Please do not write any real identifiable information about patients, hospitals, wards, colleagues, or managers.</em></li>
          </ul>
        </div>
        ...
        <textarea
          ...
          placeholder="Write your brief explanation or myth debunking here..."
          rows={4}
          style={{
            width: '100%',
            padding: '14px 16px',
          ...
          onFocus={(e) => { e.currentTarget.style.borderColor = '#006764'; }}
          onBlur={(e) => { e.currentTarget.style.borderColor = '#cbd5e1'; }}
        />
        <div style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', marginTop: '6px' }}>
          <p style={{ fontSize: '0.75rem', color: isValidInput ? '#059669' : '#94a3b8', margin: 0 }}>
            {summaryText.trim().length} characters entered {isValidInput ? '✓ (Minimum met)' : '(Minimum 25 characters)'}
          </p>
        </div>
      </div>
    </div>
  ...
  const renderFeedback = () => (
    <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 animate-fadeIn w-full max-w-4xl mx-auto">
      ...
      <div className="flex-1">
        <h2 style={STYLES.h2}>Thank You for Your Response! 🎉</h2>
        <SpeechBubble
          text={currentScene.narration}
          typing={!typingDone}
          onComplete={handleTypingComplete}
        ...
        >
          ...
          <p style={{ fontSize: '1rem', color: '#004d40', margin: 0, lineHeight: 1.7 }}>
            Thank you for your response. Summarizing learning content is a valuable way to review and organize information effectively.
          </p>
        </div>
      </div>
    </div>
          ...
          <div className="w-full max-w-4xl mt-3">
            <p style={{ fontSize: '0.8rem', color: '#f59e0b', textAlign: 'center', fontWeight: 600 }}>
              {currentScene.id === 'prompt' && !isValidInput
                ? '⚡ Please write at least 25 characters summarizing a concept or myth to continue.'
                : ''}
            </p>
          </div>
        )}
